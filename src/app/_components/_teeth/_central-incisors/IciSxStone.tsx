...
export default function IciSxStone() {
    const tooth = useTeethStore((state: State) => state.teethGeometry.icisx ? state.teethGeometry.icisx.stones : undefined);
    const toothStone =  useTeethStore((state: State) => state.teethStones.icisx);
    ...
    const ICISXstone = memo(({visible, type, mat} : {visible: boolean, type: string|undefined, mat: string}): JSX.Element => {
    ...
    return <ICISXstone visible={toothStone.shape !== undefined} type={toothStone.shape} mat={toothMaterial}/>
}