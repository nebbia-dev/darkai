...
export default function IliDxStone() {
    const tooth = useTeethStore((state: State) => state.teethGeometry.ilidx ? state.teethGeometry.ilidx.stones : undefined);
    const toothStone =  useTeethStore((state: State) => state.teethStones.ilidx);
    const ILIDXstone = memo(({visible, type} : {visible: boolean, type: string|undefined}): JSX.Element => {
    ...
    return <ILIDXstone visible={toothStone.shape !== undefined} type={toothStone.shape}/>
}